...
import { LoaderCircle, MessageSquareText, RefreshCcw, X } from "lucide-react";
...
export interface ChatWidgetProps {
  onSendMessage: (message: string, pageContext: PageContent) => void;
...
export default function ChatWidget({
  ...
  const isBusy = isLoading || isExecutingAction;
  const suggestions = [
  ...
  return (
    <ErrorBoundary onReset={onResetChat}>
      <div
            ...
            >
              <header className="flex items-center justify-between border-b border-zinc-200 bg-white px-4 py-3">
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-lg px-2 py-1.5 text-xs font-medium text-zinc-600 transition hover:bg-zinc-100 hover:text-zinc-900"
                  >
              ...
              <div className="flex min-h-0 flex-1 flex-col p-3">
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      {suggestions.map((suggestion) => (
                        <button
                          key={suggestion}
                          type="button"
                          onClick={() => onSendMessage(suggestion, pageContent)}
                ...
                <div className="mt-3 space-y-2">
                  ...
                  <ChatInput
                    onSend={(text) => onSendMessage(text, pageContent)}
                    disabled={isBusy}
                  />
                </div>
              </div>
